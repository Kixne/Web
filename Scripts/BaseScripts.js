...
/* Functions scope */{
 ...
 else{ console.info("No ServiceWorker available \n(js scope line07)"); }
 ...
 document.querySelectorAll(".copyableElement__button").forEach($copyButton => {
  $copyButton.onclick= function(){ CopyElement(this.parentElement); }
 });
 function CopyElement($copyableElementContainer){
  ...
  /* Copying the value */
  try { navigator.clipboard.writeText($copyableItem.value); }
  catch (error) {
   /* For dont supported navigator.clipboard */
   console.info("Copied with no standard code \n(js scope line44), because:", error);
   $copyableItem.classList.replace("display-n", "display-y");
   $copyableItem.select();
   document.execCommand("copy");
   $copyableItem.classList.replace("display-y", "display-n");
  }
...
/* Nav scope */{
 ...
 window.addEventListener("resize", function(){
  if(document.documentElement.clientWidth<960){
   ...
   if (isHomeSection) { $menuButton[0].classList.replace("display-n", "display-y"); }
   else{ $menuButton[1].classList.replace("display-n", "display-y"); }
  }
  if(document.documentElement.clientWidth>=960){
   $nav[1].classList.replace("display-n", "display-y");
   $nav[0].classList.replace("display-y", "display-n");
   if (isHomeSection) { $menuButton[0].classList.replace("display-y", "display-n"); }
   else{ $menuButton[1].classList.replace("display-y", "display-n"); }
  }
 });
 
 /* Menu buttons visibility toggler (mobile buttons) */
 ...
 for (let i = 1; i < 6; i++) {
  ...
  $menuItem[i].onclick= function(){
   isHomeSection= false;
   ShowMenuButton(isHomeSection);
   //when is nav course button
   if(i==1){ SwitchToCourseSection(0); }
...
/* Youtube videos scope */{
 ...
 function onPlayerStateChange(event) {
  switch (event.data){
   ...
   case 1: //1== video played
   let $playerId= event.target.getIframe().getAttribute("id");
   if ($playerId== "player-module" && player02.getPlayerState()==1) {
    player02.stopVideo();
   }
   else{
    try { //try because sometimes player01 still not created
     if ($playerId== "player-aboutUs" && player01.getPlayerState()==1) { player01.stopVideo(); }
    } catch (error) { console.info("Error catched \n(js scope line519):", error); }
   }
   break;
  }
 }
